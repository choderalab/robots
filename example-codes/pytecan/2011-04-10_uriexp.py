...
input_name = '2011-04-10_uriexp'
MES = CollectData("../res/tecan/%s.tar.gz" % input_name, number_of_plates=3)
...
for p in [2]:
    for c in xrange(4):
        vlegend = []
        for r in xrange(8):
            vlegend += [(RowCol2String(r, c), colors[r], 'solid', [(p, r, c+4*j)]) for j in xrange(3)]
        plots.append(('plate %d, column %d' % (p, c), (0, t_max), (3e-2, 0.4), 'OD600', vlegend))
