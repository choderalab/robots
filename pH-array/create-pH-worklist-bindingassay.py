...
for stage in range(nstages):
    for (condition_index, condition) in enumerate(conditions):
        ...
        worklist += aspirate(source_labware_name['citric acid'], source_labware_type['citric acid'], 1, volume, 1)
        ...
        worklist += aspirate(source_labware_name['sodium phosphate'], source_labware_type['citric acid'], 2, volume, 2)
        worklist += dispense('Buffer Mixing', '4ti-0136', destination_position, volume, 2)
        worklist += washtips()

# Write worklist.
worklist_filename = 'ph-worklist-bindingassay.gwl'
